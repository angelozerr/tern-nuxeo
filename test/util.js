...
function createServer(defs, options, isNode) {
	var plugins = isNode ? {'node': {}} : {};
	if (options) plugins['nuxeo'] = options; else plugins['nuxeo'] = {};
...
exports.assertCompletion = function(text, expected, isNode, queryOptions, pluginOptions) {
	var defs = [];
	var defNames = ["ecma5"]; 
	if (defNames) {
	...
	var server = createServer(defs, pluginOptions, isNode);
